...
NOTE: An extension can also register its own custom injectable parameter type (in addition to the built-in providers listed above) by implementing `FeatureArgumentProvider`. See xref:reference-parameter-types.adoc#custom-injectable-parameters[Custom Injectable Parameters] for details.
...
TIP: Use `@McpServer(McpServer.ALL)` to bind an extension to _all_ server configurations declared by the application without knowing their names upfront.
This is particularly useful for extensions provided by libraries.
