...
[[custom-injectable-parameters]]
== Custom Injectable Parameters

A Quarkus extension can register a custom parameter type for feature methods by implementing
`io.quarkiverse.mcp.server.FeatureArgumentProvider<T>` in its runtime module.
The provider receives the current `FeatureManager.RequestFeatureArguments` and supplies the parameter value.
These parameters are injected instead of being read from client arguments and are excluded from tool input schemas.

TIP: This is one of the building blocks for authoring a Quarkus extension on top of the MCP server. See xref:guides-mcp-extensions.adoc[MCP Extensions] for the bigger picture.
...
@Singleton
public class GreeterProvider implements FeatureArgumentProvider<Greeter> {
...
Here, `Greeter` is a custom type, for example `public record Greeter(String greeting) {}`.
Register the type and provider in the extension's deployment module:
...
class GreeterProcessor {
    ...
    @BuildStep
    FeatureArgumentProviderBuildItem greeter() {
        return new FeatureArgumentProviderBuildItem(DotName.createSimple(Greeter.class), GreeterProvider.class.getName());
    }
}
----

A feature method can then declare the registered type:
...
Providers must be CDI beans or declare a public no-argument constructor.
For CDI providers, exactly one bean must expose the provider class as a bean type; qualifiers are ignored.
The bean's context must be active during invocation, and dependent-scoped provider instances are reused.

By default, a registration applies to all features.
The three-argument `FeatureArgumentProviderBuildItem` constructor accepts a nonempty set of
`io.quarkiverse.mcp.server.Feature` values to restrict its use.
A feature method outside that set cannot declare the registered parameter type.
Built-in injectable types cannot be overridden, and each custom parameter type can have only one registration.
