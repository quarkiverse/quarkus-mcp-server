...
[source,text]
----
2024-01-15 10:23:45 INFO  [io.qua.mcp.ser.traffic] MCP message received [abc123]:
...
2024-01-15 10:23:45 INFO  [io.qua.mcp.ser.traffic] MCP message sent [abc123]:
...
=== Custom Traffic Listeners

You can implement the `io.quarkiverse.mcp.server.McpTrafficListener` interface to receive notifications when MCP messages are sent or received.
Unlike the built-in traffic logging, custom listeners are always invoked regardless of the `traffic-logging.enabled` configuration property.
...
public class MyTrafficListener implements McpTrafficListener { <1>

    @Override
    public void onMessageReceived(RawMessage message, McpConnection connection) { <2>
        // message.method() returns the JSON-RPC method name, or null for responses
        // message.id() returns the request id (RequestId), or null for notifications
        if (message.id() != null) {
            Log.infof("Request [%s] received [%s]: %s", message.method(), connection.id(), message.asString());
        }
    }
...
----
<1> A CDI bean scope is added automatically.
<2> Listener methods should be fast and non-blocking. If a listener throws an unchecked exception, it is logged and the next listener is invoked.
