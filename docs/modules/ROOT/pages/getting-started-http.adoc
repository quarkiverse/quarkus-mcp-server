...
TIP: Keep testing with `localhost` URLs. A server bound to localhost rejects requests from other origins to prevent DNS rebinding attacks, so connecting by hostname or IP address can return a `403` response. See xref:reference-security.adoc#dns-rebinding-protection[DNS Rebinding Protection].
...
* xref:guides-connecting-mcp-clients.adoc[Connect Claude Code and other MCP clients]
* xref:guides-troubleshooting.adoc[Troubleshoot connection problems]
