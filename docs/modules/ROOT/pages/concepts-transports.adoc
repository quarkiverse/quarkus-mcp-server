...
The `stdio` and `HTTP` transports differ in their deployment model, not only in their wire protocol.

With `stdio`, the client launches the server as a subprocess, so each user or session runs its own server process.
Communication never crosses the network, which means there are no proxies, no TLS, no origin checks, and no state shared between users.
The cost is that the server executable has to be distributed to each client, and a new process starts for every session.

With `HTTP`, one long-running server is shared by many clients, and requests travel across the network.
That network path introduces corporate proxies, the origin check described in xref:reference-security.adoc#dns-rebinding-protection[DNS Rebinding Protection], authentication, and TLS (see xref:reference-security.adoc[Security Reference]).
In exchange, you get central deployment, shared state, and the ability to upgrade the server independently of its clients.

Use `stdio` for local, single-user tooling such as desktop assistants, and `HTTP` for shared or remote services.
Feature methods such as `@Tool` are transport-agnostic, so switching later is a packaging and configuration change rather than a rewrite.
Note that falling back from `HTTP` to `stdio` changes the deployment model, not just the wire protocol.
...
=== Sessions, Horizontal Scaling, and Load Balancing

In the stateful mode, the `initialize`/`initialized` handshake establishes a session identified by the `Mcp-Session-Id` header.
The server keeps this session in memory, so **every subsequent request for that session must reach the same server instance**.

This becomes a problem when the server is scaled horizontally and requests are distributed across instances without session affinity — for example on serverless platforms or behind a non-sticky load balancer.
If the instance that created the session (instance A) is not the one that receives a follow-up request (instance B), instance B does not recognize the session and the request fails.

CAUTION: The symptom is easy to misdiagnose: sessions work fine with a single instance and fail only intermittently under load, typically surfacing as "unknown session" errors. If you observe this, the first thing to check is whether session affinity is actually guaranteed end-to-end.

There are two ways to address this:

Enable session affinity (sticky sessions)::
Configure your load balancer or ingress to route all requests carrying the same `Mcp-Session-Id` to the instance that owns the session.
This keeps the stateful protocol working unchanged.

Use the stateless protocol::
Stateless requests (protocol version `2026-07-28` or later) are self-contained and establish no session, so they can be handled by any instance.
This removes the affinity requirement entirely and is the recommended approach when sticky sessions cannot be guaranteed.
+
If you want to *guarantee* that clients use the stateless protocol, you can reject stateful clients with an `InitialCheck`.
Initial checks are applied only to the stateful `initialize` handshake, so stateless clients bypass them and are unaffected.
See xref:guides-using-filters-and-checks.adoc#reject-stateful-clients[Reject Stateful Clients] for a ready-to-use example.
