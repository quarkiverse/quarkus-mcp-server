...
* **Reactive types**: Methods may return `io.smallrye.mutiny.Uni<T>` or `java.util.concurrent.CompletionStage<T>` wrapping any of the supported return types for asynchronous execution
...
|Encoded via `ToolResponseEncoder` → `ContentEncoder`, then rules above apply
|Success

|`Uni<T>` or `CompletionStage<T>` wrapping any above type
