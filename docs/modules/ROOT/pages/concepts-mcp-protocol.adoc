...
With the `quarkus-mcp-server-tasks` extension, a tool can execute its work asynchronously as a task for clients that declared the https://modelcontextprotocol.io/extensions/tasks/overview[Tasks] extension (`io.modelcontextprotocol/tasks`).
The server responds immediately with a task handle and the client polls the task until it reaches a terminal status.

[mermaid,mcp-tasks-tool-call-flow,svg]
----
sequenceDiagram
    participant Client
    participant Server

    Client->>Server: 1. tools/call (name: "index", tasks capability)
    Note over Server: Task created,<br/>tool executed asynchronously
    Server-->>Client: 2. result (resultType: "task",<br/>taskId, status: "working")
    loop Poll (pollIntervalMs)
        Client->>Server: 3. tasks/get (taskId)
        Server-->>Client: 4. result (status: "working",<br/>statusMessage)
    end
    Client->>Server: 5. tasks/get (taskId)
    Server-->>Client: 6. result (status: "completed", result)
----

A task that needs input moves to the `input_required` status; the client fulfills the `inputRequests` via `tasks/update`.
A task is cancelled with `tasks/cancel`.
See xref:guides-mcp-tasks.adoc[MCP Tasks] for details.
