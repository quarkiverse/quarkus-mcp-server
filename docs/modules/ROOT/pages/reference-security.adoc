...
<3> Allow MCP Clients to discover OAuth2 Protected Resource Metadata of this Quarkus MCP Server in order to meet the https://modelcontextprotocol.io/specification/{spec-version}/basic/authorization#protected-resource-metadata-discovery-requirements[Protected Resource Metadata Discovery Requirements] of the https://modelcontextprotocol.io/specification/{spec-version}/basic/authorization[MCP Authorization specification]. See also https://quarkus.io/guides/security-oidc-expanded-configuration#resource-metadata-properties[Quarkus OIDC Expanded Configuration Guide] how to customize the protected resource metadata content.

Additionally, the `quarkus-mcp-server-oidc` extension provides automatic https://modelcontextprotocol.io/specification/{spec-version}/basic/authorization#runtime-insufficient-scope-errors[Runtime Insufficient Scope Error] responses when a client presents a valid token that lacks the required scopes.
See xref:guides-oidc.adoc[OIDC Integration] for details.
