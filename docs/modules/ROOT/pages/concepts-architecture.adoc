...
// Method returning Uni<T> or CompletionStage<T> is non-blocking by convention
@Tool
Uni<String> eventLoopTool() {
    return Uni.createFrom().item("fast");  // Runs on event loop
}

// CompletionStage<T> is also non-blocking
@Tool
CompletionStage<String> completionStageTool() {
    return CompletableFuture.completedFuture("fast");  // Runs on event loop
}


// Method returning non-async type is blocking by convention
@Tool
...
=== Async with Uni or CompletionStage

Return `Uni<T>` or `CompletionStage<T>` for truly async operations:
...
@Tool
CompletionStage<String> asyncToolWithCs() {
    return someAsyncApi.getData()  // Non-blocking call
...
|**Event Loop (Uni<T> / CompletionStage<T>)**
|Non-blocking I/O with reactive or standard JDK async APIs
|Maximum throughput, no thread overhead, true async
|Cannot perform blocking operations; use `emitOn` or `runSubscriptionOn` to switch execution context for Uni
|===
