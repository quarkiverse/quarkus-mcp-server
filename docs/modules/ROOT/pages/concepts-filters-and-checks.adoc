= Filters and Initial Checks

include::includes/attributes.adoc[]

This page explains filters and initial checks in MCP servers.

== Filters

Filters control which MCP features (tools, resources, prompts) are visible and accessible to specific clients.
They enable fine-grained control over feature availability based on connection context, client capabilities, request headers, or any other criteria.
...
* Filters are automatically discovered CDI beans
* Multiple filters are executed in order of `@Priority` (higher values execute first)
* A feature is only visible if ALL filters return `true` (logical AND)
* Filters should be lightweight as they are executed on the event loop, you cannot perform blocking operations in filters
* If a filter throws an exception, it's ignored and the next filter executes

=== Filter Interfaces

All filter interfaces follow the same pattern:
...
public interface ToolFilter {
     ...
     * @param connection The MCP connection context
...
* `initialRequest()` - Client capabilities and protocol information
...
public class FeatureFlagFilter implements ToolFilter {
    ...
    public boolean test(ToolInfo tool, McpConnection connection) {
        ...
        if (tool.name().startsWith("beta_")) {
            return isBetaEnabled(connection);
        }
        return true;
    }
...
In this example, the `SecurityFilter` executes first (higher priority). If it returns `false`, the feature is blocked regardless of what `FeatureFlagFilter` would return.
...
They provide a way to reject connections that don't meet certain requirements or to perform early validation.
...
Initial checks return a `CheckResult` indicating success or failure:
...
// Error with message
return InitialCheck.CheckResult.error("Authentication failed");
----
...
=== Accessing Initial Request Data

The `io.quarkiverse.mcp.server.InitialRequest` object provides access to:
...
public class FeatureFlagFilter implements ToolFilter, PromptFilter {
    ...
    @Override
    public boolean test(ToolInfo tool, McpConnection connection) {
        String clientId = connection.initialRequest().clientInfo().name();
