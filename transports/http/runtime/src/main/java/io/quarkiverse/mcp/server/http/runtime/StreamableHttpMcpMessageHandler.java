...
public class StreamableHttpMcpMessageHandler extends McpMessageHandler<HttpMcpRequest> implements Handler<RoutingContext> {
    ...
    private final Map<String, Boolean> toolsForceSse;
    private final Map<String, Boolean> promptsForceSse;
    private final Map<String, Boolean> resourcesForceSse;
    private final Map<String, Boolean> resourceTemplatesForceSse;
    ...
    private boolean forceSseTool(JsonObject params) {
        ...
        if (name != null) {
            Boolean forceSse = toolsForceSse.get(name);
            if (forceSse != null) {
                return forceSse;
            } else {
    ...
    private boolean forceSsePrompt(JsonObject params) {
        String name = params.getString("name");
        if (name != null) {
            Boolean forceSse = promptsForceSse.get(name);
            if (forceSse != null) {
                return forceSse;
            } else {
    ...
    private boolean forceSseResource(JsonObject params) {
        ...
        if (resourceUri != null) {
            Boolean forceSse = resourcesForceSse.get(resourceUri);
            if (forceSse != null) {
                return forceSse;
            }
            ...
            if (rti != null && rti.isMethod()) {
                forceSse = resourceTemplatesForceSse.get(rti.name());
                if (forceSse != null) {
                    return forceSse;
                }
            }
            ...
            if (info != null) {
                ...
                // Also try resource templates
                if (rti == null) {
                    rti = resourceTemplateManager.findMatching(resourceUri);
                }
                return rti != null && !rti.isMethod();
            }
        }
        return false;
    }
    ...
    private boolean forceSseCompletion(String referenceName, String argumentName,
        ...
        Boolean forceSse = completionsForceSse.get(forceSseCompletionKey(referenceName, argumentName));
        if (forceSse != null) {
            return forceSse;
        } else {
            CompletionManager.CompletionInfo info = completionManager.getCompletion(referenceName, argumentName);
            return info != null && !info.isMethod();
        }
    }

    private static String forceSseCompletionKey(String referenceName, String argumentName) {
        return referenceName + "_" + argumentName;
    }
    ...
    private static <F> Map<String, Boolean> precomputeForceSse(List<FeatureMetadata<F>> features) {
    ...
    private static <F> Map<String, Boolean> precomputeForceSse(List<FeatureMetadata<F>> features,
        ...
        for (FeatureMetadata<F> tm : features) {
            ...
            ret.put(keyMapper.apply(tm), res);
        }
        return ret;
    }
    ...
    private static <F> Map<String, Boolean> precomputeCompletionForceSse(List<FeatureMetadata<F>> features) {
        ...
        for (FeatureMetadata<F> tm : features) {
            ...
            String key = forceSseCompletionKey(tm.info().name(),
