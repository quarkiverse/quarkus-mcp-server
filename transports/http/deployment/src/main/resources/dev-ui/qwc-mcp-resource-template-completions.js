...
export class QwcMcpResourceTemplateCompletions extends LitElement {
    ...
    render() {
        if (this._completions) {
            ...
            <div style="color: var(--lumo-secondary-text-color);width: 95%;">
                <div>${msg('Fetching resource template completions...', { id: 'mcp-server-fetching-resource-template-completions' })}</div>
        ...
        return html`<div class="grid">
                    ...
                    <vaadin-grid .items="${this._filtered}" theme="row-stripes no-border" all-rows-visible
                        ...
                        <vaadin-grid-sort-column
                            header="${msg('Resource template', { id: 'mcp-server-col-resource-template' })}"
                            path="name"
                            auto-width
    ...
    _renderInputDialog() {
        return html`<vaadin-dialog
                        header-title="${msg('Complete resource template', { id: 'mcp-server-complete-resource-template' })}"
                        ...
                        ${dialogRenderer(() => this._renderCompletionInput(), [this._selectedServerName])}
                    ></vaadin-dialog>`;
    }
    ...
    _renderCompletionInput() {
        if (this._selectedCompletion) {
            ...
            return html`<vaadin-vertical-layout>
                            ...
                            <vaadin-button theme="primary" @click="${() => this._completeResourceTemplate()}">${msg('Complete', { id: 'mcp-server-complete' })}</vaadin-button>
